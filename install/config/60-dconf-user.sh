...
if [ -f ~/.local/share/omaforge/default/dconf/omaforge-gnome.ini ]; then
  # Replace placeholder with actual background URL
  temp_dconf_file=$(mktemp /tmp/omaforge-gnome.XXXXXX.ini)
  cp ~/.local/share/omaforge/default/dconf/omaforge-gnome.ini "$temp_dconf_file"
  sed -i "s|{{OMAFORGE_BACKGROUND_URL}}|file://$HOME/.config/omaforge/current/background|g" "$temp_dconf_file"
  # Load dconf settings
  dconf load / < "$temp_dconf_file" 2>/dev/null || echo "Error: Failed to load dconf settings from $temp_dconf_file"
  rm -f "$temp_dconf_file"
else
  echo "Error: dconf settings file not found at ~/.local/share/omaforge/default/dconf/omaforge-gnome.ini"
  exit 1
fi
