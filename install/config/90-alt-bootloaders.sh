...
if [ -d "/boot/loader/entries" ]; then # systemd-boot
  ...
  if ! grep -q splash /etc/cmdline.d/*.conf; then
    # Need splash, create the omaforge file
    echo "splash" | sudo tee -a /etc/cmdline.d/omaforge.conf
  fi
  if ! grep -q quiet /etc/cmdline.d/*.conf; then
    # Need quiet, create or append the omaforge file
    echo "quiet" | sudo tee -a /etc/cmdline.d/omaforge.conf
  fi
