...
# Install and configure Omaforge Plymouth theme
THEME_SOURCE_DIR="$(dirname "$(dirname "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)")")/default/plymouth"

if [ -d "$THEME_SOURCE_DIR" ]; then
  sudo mkdir -p /usr/share/plymouth/themes/omaforge/
  sudo cp -r "$THEME_SOURCE_DIR"/* /usr/share/plymouth/themes/omaforge/

  # Check if the theme is already set
  current_theme=$(readlink /etc/alternatives/default.plymouth 2>/dev/null)
  if [[ "$current_theme" != *"omaforge"* ]]; then
    # Install the theme as an alternative
    sudo update-alternatives --install /usr/share/plymouth/themes/default.plymouth default.plymouth /usr/share/plymouth/themes/omaforge/omaforge.plymouth 100

    # Set it as the default
    sudo update-alternatives --set default.plymouth /usr/share/plymouth/themes/omaforge/omaforge.plymouth
  fi
fi
